...
One such model is AUDIOSR: Versatile Audio Super-Resolution at Scale[@liu2023audioldm], which leverages a diffusion-based generative framework to reconstruct high-resolution audio from low-resolution inputs.
...
$$\mathrm{argmin}_{\mathcal{G}_{\theta}}||v_{k}-\mathcal{G}(z_{k},k,\mathcal{F}_{\mathrm{enc}}(X_{l});\theta)||_{2}^{2},$$
